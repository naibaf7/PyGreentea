...
def caffenet(netmode):
    ...
    if netmode == caffe_pb2.TEST:
        if netconf.loss_function == 'malis' or netconf.loss_function == 'malis_aniso':
            fmaps_end = 11
            
        if netconf.loss_function == 'euclid' or netconf.loss_function == 'euclid_aniso':
            fmaps_end = 11
        ...
        if netconf.loss_function == 'malis_aniso':
            net.data, net.datai = data_layer([1,1,132,132,132])
            net.label, net.labeli = data_layer([1,1,44,44,44])
            net.label_affinity, net.label_affinityi = data_layer([1,11,44,44,44])
        ...
        if netconf.loss_function == 'euclid_aniso':
            net.data, net.datai = data_layer([1,1,132,132,132])
            net.label, net.labeli = data_layer([1,11,44,44,44])
            net.scale, net.scalei = data_layer([1,11,44,44,44])
            net.silence = L.Silence(net.datai, net.labeli, net.scalei, ntop=0)
            fmaps_end = 11
...
def make_solver():
    with open('net/solver.prototxt', 'w') as f:
        ...
        print('base_lr: 0.0000005', file=f)
